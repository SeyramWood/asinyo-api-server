...
func (mq *MerchantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Merchant, error) {
	...
	if query := mq.withSupplier; query != nil {
		if err := mq.loadSupplier(ctx, query, nodes, nil,
			func(n *Merchant, e *SupplierMerchant) { n.Edges.Supplier = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withRetailer; query != nil {
		if err := mq.loadRetailer(ctx, query, nodes, nil,
			func(n *Merchant, e *RetailMerchant) { n.Edges.Retailer = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withStore; query != nil {
		if err := mq.loadStore(ctx, query, nodes, nil,
			func(n *Merchant, e *MerchantStore) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := mq.withAddresses; query != nil {
		if err := mq.loadAddresses(ctx, query, nodes,
			func(n *Merchant) { n.Edges.Addresses = []*Address{} },
			func(n *Merchant, e *Address) { n.Edges.Addresses = append(n.Edges.Addresses, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withOrders; query != nil {
		if err := mq.loadOrders(ctx, query, nodes,
	...
	if query := mq.withFavourites; query != nil {
		if err := mq.loadFavourites(ctx, query, nodes,
			func(n *Merchant) { n.Edges.Favourites = []*Favourite{} },
			func(n *Merchant, e *Favourite) { n.Edges.Favourites = append(n.Edges.Favourites, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MerchantQuery) loadSupplier(ctx context.Context, query *SupplierMerchantQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *SupplierMerchant)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_supplier" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadRetailer(ctx context.Context, query *RetailMerchantQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *RetailMerchant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_retailer" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadStore(ctx context.Context, query *MerchantStoreQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *MerchantStore)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.merchant_store
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_store" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Product)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_products" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadAddresses(ctx context.Context, query *AddressQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Address)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_addresses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (mq *MerchantQuery) loadFavourites(ctx context.Context, query *FavouriteQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *Favourite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Merchant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_favourites" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
