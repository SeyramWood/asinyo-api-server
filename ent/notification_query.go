...
func (nq *NotificationQuery) QueryAdmin() *AdminQuery {
	query := (&AdminClient{config: nq.config}).Query()
...
func (nq *NotificationQuery) QueryMerchant() *MerchantQuery {
	query := (&MerchantClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) QueryAgent() *AgentQuery {
...
func (nq *NotificationQuery) QueryCustomer() *CustomerQuery {
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withAdmin:    nq.withAdmin.Clone(),
		withMerchant: nq.withMerchant.Clone(),
		withAgent:    nq.withAgent.Clone(),
...
func (nq *NotificationQuery) WithAdmin(opts ...func(*AdminQuery)) *NotificationQuery {
	query := (&AdminClient{config: nq.config}).Query()
...
func (nq *NotificationQuery) WithMerchant(opts ...func(*MerchantQuery)) *NotificationQuery {
	query := (&MerchantClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withMerchant = query
	return nq
}
...
func (nq *NotificationQuery) WithAgent(opts ...func(*AgentQuery)) *NotificationQuery {
	query := (&AgentClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (nq *NotificationQuery) WithCustomer(opts ...func(*CustomerQuery)) *NotificationQuery {
	query := (&CustomerClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := nq.withAdmin; query != nil {
		if err := nq.loadAdmin(ctx, query, nodes,
			func(n *Notification) { n.Edges.Admin = []*Admin{} },
			func(n *Notification, e *Admin) { n.Edges.Admin = append(n.Edges.Admin, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withMerchant; query != nil {
		if err := nq.loadMerchant(ctx, query, nodes,
			func(n *Notification) { n.Edges.Merchant = []*Merchant{} },
			func(n *Notification, e *Merchant) { n.Edges.Merchant = append(n.Edges.Merchant, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withAgent; query != nil {
		if err := nq.loadAgent(ctx, query, nodes,
			func(n *Notification) { n.Edges.Agent = []*Agent{} },
			func(n *Notification, e *Agent) { n.Edges.Agent = append(n.Edges.Agent, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withCustomer; query != nil {
		if err := nq.loadCustomer(ctx, query, nodes,
			func(n *Notification) { n.Edges.Customer = []*Customer{} },
			func(n *Notification, e *Customer) { n.Edges.Customer = append(n.Edges.Customer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadAdmin(ctx context.Context, query *AdminQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Admin)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.AdminTable)
		s.Join(joinT).On(s.C(admin.FieldID), joinT.C(notification.AdminPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.AdminPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.AdminPrimaryKey[1]))
...
func (nq *NotificationQuery) loadMerchant(ctx context.Context, query *MerchantQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Merchant)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Notification)
	nids := make(map[int]map[*Notification]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.MerchantTable)
		s.Join(joinT).On(s.C(merchant.FieldID), joinT.C(notification.MerchantPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.MerchantPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.MerchantPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Merchant](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "merchant" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (nq *NotificationQuery) loadAgent(ctx context.Context, query *AgentQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Agent)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Notification)
	nids := make(map[int]map[*Notification]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.AgentTable)
		s.Join(joinT).On(s.C(agent.FieldID), joinT.C(notification.AgentPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.AgentPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.AgentPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "agent" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (nq *NotificationQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Customer)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Notification)
	nids := make(map[int]map[*Notification]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.CustomerTable)
		s.Join(joinT).On(s.C(customer.FieldID), joinT.C(notification.CustomerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.CustomerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.CustomerPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
