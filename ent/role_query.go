...
func (rq *RoleQuery) QueryAdmin() *AdminQuery {
...
func (rq *RoleQuery) WithAdmin(opts ...func(*AdminQuery)) *RoleQuery {
	query := (&AdminClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withAdmin; query != nil {
		if err := rq.loadAdmin(ctx, query, nodes,
			func(n *Role) { n.Edges.Admin = []*Admin{} },
			func(n *Role, e *Admin) { n.Edges.Admin = append(n.Edges.Admin, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadAdmin(ctx context.Context, query *AdminQuery, nodes []*Role, init func(*Role), assign func(*Role, *Admin)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Role)
	nids := make(map[int]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.AdminTable)
		s.Join(joinT).On(s.C(admin.FieldID), joinT.C(role.AdminPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.AdminPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.AdminPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
