...
func (ru *RoleUpdate) AddAdmin(a ...*Admin) *RoleUpdate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (ru *RoleUpdate) ClearAdmin() *RoleUpdate {
	ru.mutation.ClearAdmin()
	return ru
}
...
func (ru *RoleUpdate) RemoveAdmin(a ...*Admin) *RoleUpdate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (ru *RoleUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if ru.mutation.AdminCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   role.AdminTable,
			Columns: role.AdminPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
	...
	if nodes := ru.mutation.RemovedAdminIDs(); len(nodes) > 0 && !ru.mutation.AdminCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   role.AdminTable,
	...
	if nodes := ru.mutation.AdminIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   role.AdminTable,
...
// AddAdmin adds the "admin" edges to the Admin entity.
func (ruo *RoleUpdateOne) AddAdmin(a ...*Admin) *RoleUpdateOne {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
// ClearAdmin clears all "admin" edges to the Admin entity.
func (ruo *RoleUpdateOne) ClearAdmin() *RoleUpdateOne {
	ruo.mutation.ClearAdmin()
	return ruo
}
...
// RemoveAdmin removes "admin" edges to Admin entities.
func (ruo *RoleUpdateOne) RemoveAdmin(a ...*Admin) *RoleUpdateOne {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (ruo *RoleUpdateOne) sqlSave(ctx context.Context) (_node *Role, err error) {
	...
	if nodes := ruo.mutation.RemovedAdminIDs(); len(nodes) > 0 && !ruo.mutation.AdminCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   role.AdminTable,
	...
	if nodes := ruo.mutation.AdminIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   role.AdminTable,
