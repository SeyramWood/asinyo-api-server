...
func (odq *OrderDetailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderDetail, error) {
	...
	if query := odq.withProduct; query != nil {
		if err := odq.loadProduct(ctx, query, nodes, nil,
			func(n *OrderDetail, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := odq.withStore; query != nil {
		if err := odq.loadStore(ctx, query, nodes, nil,
			func(n *OrderDetail, e *MerchantStore) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (odq *OrderDetailQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*OrderDetail, init func(*OrderDetail), assign func(*OrderDetail, *Order)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_details" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (odq *OrderDetailQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*OrderDetail, init func(*OrderDetail), assign func(*OrderDetail, *Product)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OrderDetail)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_order_details" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (odq *OrderDetailQuery) loadStore(ctx context.Context, query *MerchantStoreQuery, nodes []*OrderDetail, init func(*OrderDetail), assign func(*OrderDetail, *MerchantStore)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OrderDetail)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "merchant_store_order_details" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
