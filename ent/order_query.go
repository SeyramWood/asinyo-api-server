...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withMerchant; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "merchant_orders" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withAgent; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "agent_orders" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withAddress; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "address_orders" returned %v`, n.ID)
			}
			for i := range nodes {
