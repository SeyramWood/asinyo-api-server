...
func (*MerchantStore) scanValues(columns []string) ([]any, error) {
...
func (ms *MerchantStore) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case merchantstore.FieldPermitAgent:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field permit_agent", values[i])
			} else if value.Valid {
				ms.PermitAgent = value.Bool
			}
...
func (ms *MerchantStore) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("permit_agent=")
	builder.WriteString(fmt.Sprintf("%v", ms.PermitAgent))
	builder.WriteByte(')')
