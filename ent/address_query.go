...
func (aq *AddressQuery) sqlAll(ctx context.Context) ([]*Address, error) {
	...
	if query := aq.withMerchant; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "merchant_addresses" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withAgent; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "agent_addresses" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withCustomer; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_addresses" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withOrders; query != nil {
		...
		for _, n := range neighbors {
			fk := n.address_orders
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "address_orders" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "address_orders" returned %v for node %v`, *fk, n.ID)
			}
