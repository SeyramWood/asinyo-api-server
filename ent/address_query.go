...
func (aq *AddressQuery) QueryMerchant() *MerchantQuery {
	query := &MerchantQuery{config: aq.config}
...
func (aq *AddressQuery) QueryAgent() *AgentQuery {
	query := &AgentQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AddressQuery) QueryOrders() *OrderQuery {
...
func (aq *AddressQuery) Clone() *AddressQuery {
	...
	return &AddressQuery{
		...
		withMerchant: aq.withMerchant.Clone(),
...
func (aq *AddressQuery) WithMerchant(opts ...func(*MerchantQuery)) *AddressQuery {
	query := &MerchantQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withMerchant = query
	return aq
}
...
func (aq *AddressQuery) WithAgent(opts ...func(*AgentQuery)) *AddressQuery {
	query := &AgentQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AddressQuery) WithOrders(opts ...func(*OrderQuery)) *AddressQuery {
	query := &OrderQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Address, error) {
	...
	if aq.withMerchant != nil || aq.withAgent != nil || aq.withCustomer != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withMerchant; query != nil {
		if err := aq.loadMerchant(ctx, query, nodes, nil,
			func(n *Address, e *Merchant) { n.Edges.Merchant = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withAgent; query != nil {
		if err := aq.loadAgent(ctx, query, nodes, nil,
			func(n *Address, e *Agent) { n.Edges.Agent = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := aq.withOrders; query != nil {
		if err := aq.loadOrders(ctx, query, nodes,
			func(n *Address) { n.Edges.Orders = []*Order{} },
			func(n *Address, e *Order) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AddressQuery) loadMerchant(ctx context.Context, query *MerchantQuery, nodes []*Address, init func(*Address), assign func(*Address, *Merchant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Address)
	for i := range nodes {
		if nodes[i].merchant_addresses == nil {
			continue
		}
		fk := *nodes[i].merchant_addresses
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "merchant_addresses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AddressQuery) loadAgent(ctx context.Context, query *AgentQuery, nodes []*Address, init func(*Address), assign func(*Address, *Agent)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Address)
	for i := range nodes {
		if nodes[i].agent_addresses == nil {
			continue
		}
		fk := *nodes[i].agent_addresses
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "agent_addresses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AddressQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Address, init func(*Address), assign func(*Address, *Customer)) error {
	...
	nodeids := make(map[int][]*Address)
	for i := range nodes {
		if nodes[i].customer_addresses == nil {
			continue
		}
		fk := *nodes[i].customer_addresses
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_addresses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AddressQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Address, init func(*Address), assign func(*Address, *Order)) error {
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(address.OrdersColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.address_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "address_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "address_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
